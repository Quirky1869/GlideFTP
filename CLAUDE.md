...
- `issues-to-github.txt` - detailed technical write-up of every issue (title, labels, description, solution). Currently covers #1-#97. Add new entries after each fix.
...
# AppImage archives (Arch AND Debian) also include a README.md (staged via mktemp -d, same as the
#   Linux binary archive) - all three archive types share one uniform README template (see
#   "Uniform archive README template" below), only the executable name and title differ
...
> `go.mod` pins `github.com/wailsapp/wails/v2 v2.15.0` (bumped from v2.12.0 during the v1.7.7 sound-notification work, to match the locally-installed `wails` CLI - `wails dev`/`build` auto-`go mod tidy` to the CLI's version). Confirmed building and running cleanly (full manual pass: connect UI, settings, transfers, the new sound feature) - GlideFTP only uses long-stable v2 APIs (`wails.Run`, `pkg/options`, `pkg/runtime` dialogs/events/window calls), none of which changed across this range.
...
└── frontend/src/
    ...
    │   └── notify.js               # Global notification popup store - notify(message, type) / closeNotify(); replaces native alert()
    ├── assets/
    │   └── sounds/                 # chime.wav, bell.wav, pop.wav, glass.wav - reference copies only (for listening), NOT imported by the app; generated by scripts/generate_notification_sounds.py, never hand-edited
    ├── utils/
    │   ├── focusTrap.js            # Svelte action `trapFocus` - traps Tab/Shift+Tab inside a popup container
    │   ├── sound.js                # NOTIFICATION_SOUNDS list + playNotificationSound(id) - plays a data: URI (see sounds.generated.js) via `new Audio()`
    │   └── sounds.generated.js     # AUTO-GENERATED - base64 data: URIs of the .wav files above; this is what sound.js actually imports (see WebKitGTK gotcha below)
    └── components/
        ...
        ├── SettingsPanel.svelte    # Sliding panel (75% width from right); footer shows version badge (accent color, bottom-left) - update hardcoded version string on each release; reset buttons (↺) next to each label, shown only when the field differs from its default (see below); window size + maximize settings
        ...
        ├── NotifyModal.svelte      # Global themed popup (accent border, no icon, large bold message) - mounted once in App.svelte, replaces native alert() everywhere
...
- **Reorder sites via drag-and-drop** (v1.7.7, `SiteManager.svelte`): a square toggle button to the right of "+ Nouveau site" - both wrapped in `.site-list-toolbar` - flips `reorderMode`. Icon swaps with the state: up/down chevrons when off, a checkmark when on (title tooltip switches from `reorderSites` to `reorderSitesDone` too) - reinforces that clicking it again is how you confirm/exit reorder mode. Order is implicit in array position (no `Order` field on `sites.Site`): `internal/sites/sites.go`'s `Reorder(orderedIDs []string)` rebuilds `m.sites` to match, keeping any site missing from the list appended at the end (defensive against a stale/partial frontend payload) and re-`save()`s; exposed as `app.ReorderSites(orderedIDs)`. While `reorderMode` is true, each `.site-item` gets `draggable={true}` plus a 6-dot grip icon (`.drag-handle`) and its normal click-to-select is disabled (`if (reorderMode) return;` in the click handler) so dragging can't accidentally open a site's detail view - same intra-app HTML5 drag-and-drop pattern already proven on WebKit-GTK by `FileBrowser`'s own row dragging. `handleSiteDrop` splices the dragged site to its new index in the local `sites` array (instant visual feedback) then calls `ReorderSites(sites.map(s => s.id))` to persist. `openExportDialog()` forces `reorderMode = false` so the two modes (export-select vs. reorder) can't be active at once - both repurpose the same toolbar/row area. `reorderMode` needs no explicit reset elsewhere: `SiteManager` is fully destroyed/recreated (`{#if showSiteManager}`) each time the modal closes and reopens.
...
- **Delete-site confirmation layout** (v1.7.7, fixes issue #36 from the v1.7.6 batch): clicking "Supprimer le site" used to cram the confirmation text and Oui/Non buttons into the *same* `.view-actions` flex row as Connecter/Modifier le site, wrapping unevenly (text off-center, Oui/Non only under "Modifier le site", not the whole row). Now it's three separate stacked blocks, all direct children of `.site-view` (a `flex-direction: column; align-items: center` container, so each is centered and evenly gapped for free): (1) `.view-actions` with just Connecter + Modifier le site (Supprimer le site is hidden while confirming, via `{#if confirmDeleteId !== selectedSite.id}`); (2) a `<p class="confirm-text">` with the "Êtes-vous sûr..." message on its own centered line; (3) a second `.view-actions` div with only Oui/Non. `confirmDeleteId` state and the `deleteSite()` handler are unchanged - purely a template/CSS reshuffle.
...
- **"Test connection" button** (v1.7.7, `SiteManager.svelte` + `app.go`): full-width button between the Note `.form-row` and `.form-actions` in the create/edit form. `app.TestConnection(cfg connection.Config) error` builds a standalone `FTPClient`/`SFTPClient` (via the existing `NewFTPClient`/`NewSFTPClient`), calls `Connect()` then `Disconnect()` - it never touches `connMgr`, so it can't affect or drop any tab already open. Bounded by a dedicated `testConnectionTimeoutSec = 10` constant (independent of the user-configurable "Operation timeout" setting, which can be 60s+) via a watchdog goroutine/channel/`select` raced against `time.After(10s)` - copied from `Manager.ListDir`'s pattern - because the dial timeout alone doesn't cover a server that accepts the TCP connection but hangs during login/handshake; a detached goroutine still disconnects a late-arriving success so no socket leaks. Frontend: `testState` (`'idle'|'testing'|'success'|'error'`) drives a `{#key testState}` block that swaps the label for a looping 3-dot animation (`test-dot-pulse` keyframe, staggered `animation-delay` per dot) while testing, then a green/red result (`connectionOk`/`connectionFailed` i18n keys) - each transition gets a `test-content-in` fade/slide keyframe (0.25s), no animation library, consistent with the rest of the app. Auto-reverts to `'idle'` after 3s (cleared in `onDestroy`); a reactive block also resets to `'idle'` the instant any connection-relevant form field changes after a test, so a stale result is never shown against an edited, untested config. Styling: `.btn-test` matches `.btn-secondary` (neutral, not accent-colored) by default; `.test-success`/`.test-error` set `var(--success)`/`var(--danger)` - their `:hover` variants must be declared *after* the generic `.btn-test:hover:not(:disabled)` rule (both have identical CSS specificity `(0,3,0)`, so source order decides the winner) or hovering during/after a test silently keeps the idle color instead of turning green/red.
- **Color picker "Save" applies immediately** (v1.7.7, `SettingsPanel.svelte`): `onColorApply(hex)` (passed to `<ColorPicker onApply={onColorApply}>`) used to only update the local `form.accentColor` and close the picker, leaving the color unpersisted until the user clicked the outer panel's own "Enregistrer" a second time. It now mirrors `save()` exactly - sets the form value, closes the picker, then `await saveSettings(form)` (persists via `SaveSettings`, updates the `settings` store, calls `applyAccentColor()` for the live CSS vars), shows the same `saved` confirmation, and calls `onSaved(form)` - one click in the picker is enough.
...
- **Transfer-complete sound notification** (v1.7.7): `settings.NotificationSoundEnabled` (bool, default `false`) + `settings.NotificationSound` (string id, default `"chime"`) in `internal/settings/settings.go` - plain JSON fields, no Go-side audio playback involved, no validation of the id (any unknown id just falls back to `NOTIFICATION_SOUNDS[0]` in `playNotificationSound`). Actual playback happens purely in the frontend: `frontend/src/utils/sound.js` exports `NOTIFICATION_SOUNDS` (13 sounds - the original 4 synthesized ones `chime`/`bell`/`pop`/`glass` plus 9 added later from user-supplied recordings: `siren`/`buzzer`/`alert`/`alarm`/`ring`/`trill`/`ping`/`beep`/`signal`) and `playNotificationSound(id)` (`new Audio(url).play()`, one cached `Audio` instance per sound kept alive for the app's lifetime - a bare `new Audio(url).play()` with no reference held can get garbage-collected by WebKit's JS engine before playback starts). It fires once per **queue drain**, not per file: `stores/transfers.js` subscribes to the `transfers` store and tracks `hadActiveJobs` (any job `pending`/`running`); the sound plays only on the transition from "had active jobs" to "no active jobs left" (covers uploads and downloads alike, one sound per batch, right after the last job wraps up), gated on `get(settings).notificationSoundEnabled`. No in-app volume control - the `Audio` element plays at its default volume and the OS/app mixer governs actual loudness, per the user's request. UI in `SettingsPanel.svelte` > Notifications: on/off switch (`sw` pattern); when on, a `<select>` dropdown of sound names plus a speaker-icon button that calls `playNotificationSound(form.notificationSound)` to preview - `<select>` is a proven-safe pattern on WebKit-GTK already used in `ConnectionBar`/`SiteManager`, unlike hidden checkboxes or native number spinners. The dropdown options are **not** rendered straight from `NOTIFICATION_SOUNDS`'s fixed array order: `SettingsPanel.svelte` computes `sortedSounds` reactively (`$: sortedSounds = [...NOTIFICATION_SOUNDS].sort((a, b) => $t(a.label).localeCompare($t(b.label), $locale))`) so the list is alphabetical in whichever language is active - en and fr order differently (e.g. `alarm/alert/beep/bell/...` vs `alarme/alerte/bip/buzzer/...`), so a single static sort would only ever be correct for one language. `locale` is imported from `i18n/index.js` alongside `t`.
  - **WebKitGTK Linux gotcha - `<audio>`/`<video>` cannot use Wails' custom URI scheme.** Sounds are generated by `scripts/generate_notification_sounds.py` (pure-stdlib Python synth: soft attack + exponential decay, no external deps) which writes both `frontend/src/assets/sounds/*.wav` (reference files, for listening) and `frontend/src/utils/sounds.generated.js` (the same sounds as `data:audio/wav;base64,...` strings - the one actually imported by `sound.js`). The `.wav` files are **not** imported directly: on Linux, Wails' native window always loads the frontend through its own custom `wails://` URI scheme (`internal/frontend/desktop/linux/frontend.go`'s `startURL`) - true even in `wails dev`, where the printed `http://localhost:34115` is only for manually opening the page in a real browser to debug, not what the app window itself uses. WebKitGTK's HTML5 media element loader recognizes only a fixed set of schemes (`http`/`https`/`file`/`data`/`blob`/...) before ever touching GStreamer; `wails://` isn't one of them, so a plain `<audio src>`/`Audio(url)` pointing at a `wails://...` asset fails instantly (`MediaPlayerPrivateGStreamer.cpp:1554:loadingFailed: ... FormatError`, confirmed via `GST_DEBUG=3,webkitwebsrc:6,souphttpsrc:6,typefind:5` showing zero output from any of those categories - the failure happens before a GStreamer pipeline is even built) - reproduces identically in `wails dev` and the packaged binary, on both the assets and hardware-acceleration/sandbox axes (`WEBKIT_DISABLE_SANDBOX=1`, `WEBKIT_DISABLE_COMPOSITING_MODE=1` do not fix it - this is not a GPU/driver/sandbox issue). `data:` URIs sidestep the scheme check entirely. Vite 3 (pinned by this project) has no dev-server-side asset inlining (`?inline` and `assetsInlineLimit` only apply to `vite build`, not `vite dev`), hence generating the base64 JS module ahead of time instead of relying on Vite's own inlining. **Any future audio/video element must go through this same `sounds.generated.js`-style `data:` URI pattern - a plain imported asset URL will silently fail on Linux.**
  - **Adding a new notification sound**: `generate_notification_sounds.py` only knows how to (re)synthesize the 4 original built-in sounds from scratch - it has no "import an external audio file" mode. To add one supplied by the user (any format - mp3/wav/ogg/m4a/...): (1) convert it with `ffmpeg` to mono 16-bit PCM WAV at 44100 Hz, normalizing its peak level to match the others (measure with `ffmpeg -i in.wav -af volumedetect -f null -` then apply `-af "volume=XdB"` to bring `max_volume` to -5.2 dB, the level all 13 current sounds share); (2) base64-encode the result and add it as a new key under `SOUND_DATA_URLS` in `frontend/src/utils/sounds.generated.js`; (3) add a matching `{id, label, url}` entry to `NOTIFICATION_SOUNDS` in `frontend/src/utils/sound.js` - the id must not reuse a word already used by an existing sound id; (4) add the `soundXxx` i18n label to both `frontend/src/i18n/en.js` and `fr.js` (translate the label, don't just copy the English word - it feeds the alphabetical sort described above); (5) also save a copy to `frontend/src/assets/sounds/<id>.wav` (reference file, for listening - matches the pattern of the original 4, even though it's hand-converted rather than script-generated for these). Never reference the user-supplied file as a plain imported asset - see the WebKitGTK gotcha above for why that fails silently on Linux. Precedent: the 9 sounds beyond the original 4 (`siren`/`buzzer`/`alert`/`alarm`/`ring`/`trill`/`ping`/`beep`/`signal`) were added this way from user-supplied `.wav` recordings, no code changes needed on the Go side (the setting is an unvalidated string id).
...
- **Settings reset buttons conditionally shown** (`SettingsPanel.svelte`): each field's `↺` reset button (`class="reset-btn"`, calls `resetSetting(key)`) has `hidden={form.<key> === DEFAULTS.<key>}` - it only renders when the field's current value differs from `DEFAULTS[key]`, and disappears again as soon as the value matches the default (via reset or manual edit). Plain HTML `hidden` attribute, no `display` override in `.reset-btn` CSS to fight it. All 20 reset buttons follow this pattern - add `hidden={form.newKey === DEFAULTS.newKey}` for any new resettable setting.
- **Settings export/import** (v1.7.7, issue #94): `app.ExportSettings()` (SaveFileDialog → writes `a.appSettings` as indented JSON, no passwords/site data involved - unrelated to the sites `.gfe` export) and `app.ImportSettings() (*settings.Settings, error)` (OpenFileDialog → unmarshals onto `settings.Default()` so a partial/older export doesn't zero out missing fields → applies immediately via `queue.SetWorkers`/`SetSpeedLimit` → persists via `Settings.Save()` → returns the resolved struct). `SettingsPanel.svelte`'s `exportSettings()`/`importSettings()` call these; import also calls the existing `saveSettings()` store helper so theme/locale/accent/`form` update live, no restart needed. UI: two icon-only round buttons (`.io-icon-btn`, 30px circle) in the panel footer, right after the version badge, `title` shows "Exporter/Importer les paramètres" on hover - up-arrow icon = Export (stays leftmost), down-arrow icon = Import; the same two icons (same meaning) were applied to the existing Export/Import buttons in `SiteManager.svelte`'s "Gérer les sites" header for consistency.
- **Local path environment-variable expansion** (v1.7.7, issue #95): `localfs.ExpandPath(path string) string` (`internal/fs/local.go`) expands a leading `~`/`~/`, Windows `%VAR%` (regexp, via `os.LookupEnv`), and Unix `$VAR`/`${VAR}` (via `os.Expand`) - an unset variable is left untouched rather than collapsed to `""`. Exposed as `app.ExpandLocalPath(path)`, a pure string transform that never touches the filesystem. Deliberately called at only two places, never inside `ListDir` or the shared `onNavigate` used by every directory click: `initLocalDir()` in `connection.js` (the "Default local directory" setting, at startup and on live-apply) and `FileBrowser.svelte`'s `confirmEditPath()` (the editable path bar's Enter-submit), gated on `side === 'local'` - a remote path like `$logs` is a literal server directory name. This avoids mangling navigation into a real folder whose name contains `$`/`%` (e.g. Windows' `$Recycle.Bin`). Saving a new "Default local directory" in Settings now also navigates the local panel there immediately: `App.svelte`'s `handleSettingsSaved(newSettings)` tracks `lastDefaultLocalDir` and calls `initLocalDir(newValue)` when it changed, instead of requiring an app restart.
- **Global notification popup** (v1.7.7, issue #96): `stores/notify.js` (`notification` writable store + `notify(message, type='success'|'error')` / `closeNotify()`) and `components/NotifyModal.svelte` replace every native `alert()` in the app (there is none left - verified by grep) with a themed, translated modal: centered overlay, 2px `var(--accent)` border + accent-tinted glow, large bold centered message (19px/700, no icon; `var(--danger)` text color when `type: 'error'`), `use:trapFocus`, closes on backdrop click / "Fermer" button / Escape (wired into `App.svelte`'s global `handleKeydown`, checked before Settings/SiteManager). Mounted once at the root in `App.svelte` with `z-index: 5000` so it always sits above every other panel. Used by `SettingsPanel.svelte` (export/import errors, "Paramètres importés") and `SiteManager.svelte` (connect failures via the `connectError` i18n key instead of a hardcoded English fallback, export/import errors, "N sites importés").
- **Release notes**: one `v{version}.md` file per release at project root (EN + FR, features + bugfixes + download table). `SettingsPanel.svelte` footer badge must be updated to match on each release. File starts with `## 🇬🇧 English` (no `#` title - format validated by user on v1.7.5). The download table's last row is `Arch via AUR | yay -S glideftp-bin` (same wording EN and FR) - unlike every other row it's an install command, not a downloadable file, since AUR builds from the published archive rather than shipping its own artifact.
...
- **Draft release title format**: `V{version}` (capital V, no "GlideFTP" prefix, e.g. `V1.7.6`) - matches the convention of every past release. Set via `name: "V${{ steps.ver.outputs.version }}"` in `release.yml`'s `softprops/action-gh-release@v2` step - do not change to `"GlideFTP v..."` or lowercase `v...`.
- **create-archive.sh Arch AppImage graceful skip**: `make_appimage_arch_gz`/`make_appimage_arch_tar` check the Arch AppImage binary exists before copying it, and warn+`return` instead of aborting if missing (same pattern as `copy_packages()`). Required because CI's default `-p all` run in `create-archive.sh` never builds the Arch AppImage (Arch-only libs) - without the guard, CI's "Create distribution archives" step fails with `cp: cannot stat ...AppImage`.
- **AUR publishing workflow**: the full step-by-step process (one-time SSH key setup for the AUR account + the repeatable per-release checklist: tag/push, watch CI, build the Arch AppImage locally, publish the draft release, verify the real sha256 of the published asset, update `packaging/PKGBUILD` if needed, `makepkg -si` local test, then commit+push `PKGBUILD`+`.SRCINFO` to the `glideftp-bin` AUR repo) is documented in `memo/workflow-aur.md` - keep it in sync whenever the release process changes. Two gotchas recorded there: the AUR clone must be a real `git clone` of `ssh://aur@aur.archlinux.org/glideftp-bin.git` (never a manually-created folder - `git push` has nowhere to go otherwise); and after a package's very first-ever publish, `yay`/AUR helpers can report "target not found" for a few minutes while the RPC index catches up, even though the package's web page is already live.
...
- **Uniform archive README template** (`create-archive.sh`): all three Linux archive README writers - `_write_linux_readme`, `_write_arch_appimage_readme`, `_write_debian_appimage_readme` - share the exact same EN-then-FR section structure: `## GlideFTP - Linux Prerequisites` (webkit2gtk install per distro + `chmod +x <exe>`/`./<exe>`) → a "run it" section (`## Running the binary executable` for the plain binary, `## Running the AppImage` for both AppImage variants - same chmod/run commands repeated) → `## Password Storage - Keyring`. Only the title (`Linux Binary` / `AppImage Arch` / `AppImage Debian`) and the executable name (`GlideFTP` / `GlideFTP-Arch-x86_64.AppImage` / `GlideFTP-Debian-x86_64.AppImage`, used in *both* the prerequisites chmod line and the dedicated run section) differ between the three. Keep all three in sync when editing one - the user has asked twice for byte-for-byte consistency across archives. The keyring check block is identical in all three: `pgrep -f gnome-keyring-daemon || pgrep -f kwalletd6 || echo "no keyring daemon found"` (FR: `"aucun daemon keyring trouve"`) then a line literally reading `or` (FR: `ou`) then `pacman -Qs gnome-keyring ; pacman -Qs kwallet` - this mirrors the keyring section already in the root `README.md` (`#### Linux - password storage (keyring)` EN / `#### Linux - stockage des mots de passe (keyring)` FR). Debian/Arch AppImage archives are staged via `mktemp -d` (like the Linux binary archive already did) so the README can be added to the tar alongside the `.AppImage`.
