...
- **Transfer-complete sound notification** (v1.7.7): `settings.NotificationSoundEnabled` (bool, default `false`) + `settings.NotificationSound` (string id, default `"chime"`) in `internal/settings/settings.go` - plain JSON fields, no Go-side audio playback involved. Actual playback happens purely in the frontend: `frontend/src/utils/sound.js` exports `NOTIFICATION_SOUNDS` (`chime`/`bell`/`pop`/`glass`) and `playNotificationSound(id)` (`new Audio(url).play()`, one cached `Audio` instance per sound kept alive for the app's lifetime - a bare `new Audio(url).play()` with no reference held can get garbage-collected by WebKit's JS engine before playback starts). It fires once per **queue drain**, not per file: `stores/transfers.js` subscribes to the `transfers` store and tracks `hadActiveJobs` (any job `pending`/`running`); the sound plays only on the transition from "had active jobs" to "no active jobs left" (covers uploads and downloads alike, one sound per batch, right after the last job wraps up), gated on `get(settings).notificationSoundEnabled`. No in-app volume control - the `Audio` element plays at its default volume and the OS/app mixer governs actual loudness, per the user's request. UI in `SettingsPanel.svelte` > Notifications: on/off switch (`sw` pattern); when on, a `<select>` dropdown of sound names plus a speaker-icon button that calls `playNotificationSound(form.notificationSound)` to preview - `<select>` is a proven-safe pattern on WebKit-GTK already used in `ConnectionBar`/`SiteManager`, unlike hidden checkboxes or native number spinners.
  ...
  - **Adding a new notification sound**: `generate_notification_sounds.py` only knows how to (re)synthesize the 4 built-in sounds from scratch - it has no "import an external audio file" mode. To add one supplied by the user (any format - mp3/wav/ogg/m4a/...): (1) convert it with `ffmpeg` to mono 16-bit PCM WAV at 44100 Hz, normalizing its peak level so it's not jarringly louder/quieter than the other 4; (2) base64-encode the result and add it as a new key under `SOUND_DATA_URLS` in `frontend/src/utils/sounds.generated.js`; (3) add a matching `{id, label, url}` entry to `NOTIFICATION_SOUNDS` in `frontend/src/utils/sound.js`; (4) add the `soundXxx` i18n label to both `frontend/src/i18n/en.js` and `fr.js`. Never reference the user-supplied file as a plain imported asset - see the WebKitGTK gotcha above for why that fails silently on Linux.
