...
└── frontend/src/
    ...
    └── components/
        ...
        ├── FileBrowser.svelte      # Single panel: nav, sort, multi-select, drag-drop, rename, delete; tree view toggle (lazy-loaded hierarchy, files + dirs, per-panel state)
        ├── TransferQueue.svelte    # Bottom panel, resizable, 3 tabs: pending/failed/done
        ├── SettingsPanel.svelte    # Sliding panel (75% width from right); footer shows version badge (accent color, bottom-left) - update hardcoded version string on each release
